...
Описание
--------
Программа предназначена для динамической распаковки биндеров, криптеров, пакеров и протекторов:
...
Zylom Wrapper
------------------------------
2981 известных биндеров, криптеров, пакеров и протекторов и много других...
...
QuickUnpack пытается обходить всевозможные скрэмблеры/обфускаторы и восстанавливать перенаправленный импорт. С версии 1 добавлена возможность распаковки dll. С версии 2 добавлена возможность аттача к процессу, что позволяет использовать QuickUnpack как дампер и восстановитель импорта. Также в версии 2 добавлена возможность использования скриптов, что позволяет распаковывать ещё более сложные протекторы. А с 3 версии добавлена поддержка x64 и отладочного ядра на аппаратной виртуализации. Эти возможности делают QuickUnpack уникальным программным продуктом, который не имеет подобных аналогов в мире!
...
Если у Вас есть предложения по развитию данной программы или Вы нашли какие-либо баги, просьба сообщить мне об этом. Найти меня на форуме exelab.ru или на форуме exetools.com или на форуме tport.org

Лицензия и права
----------------
Список исходников, использовавшихся при написании QuickUnpack:
stripper распаковщик asprotect от syd
GenOEP.dll от Snaker
дизассемблер Mediana от Mikae
Marquee бегущая строка от Funbit/TSRh
плагины и некоторый другой код от NEOx
MiniFMOD от Firelight Technologies
код SEException от Martin Ziacek
Picture (Implementations) от Dr. Yovav Gad
для OEP finders автор указан в самой программе
...
v4.2 [14.10.2016]
[!] исправлены многие ошибки и сделаны улучшения
[+] добавлена новая сортировка импорта
[+] добавлен хардкорный метод использования старой IAT
[+] обновлен дизассемблер
[+] расширен список протекторов
...
[+] добавлено отладочное ядро на аппаратной виртуализации Intel
...
[+] закончена работа над менеджером памяти, который позволит собирать всю динамически выделенную память в один кусок (только в private версии)
[+] добавлено несколько новых функций и переменных для скриптов
...
[+] добавлена возможность импорта функций из ImpRec (теперь QuickUnpack поддерживает экспорт и импорт функций импорта в ImpRec-совместимые файлы, что позволяет править функции руками или добавлять новые. однако следует помнить, что это дело работает с нормально созданными файлами, но если туда напихать мусор или по-извращенски файл отформатировать, всё может упасть :) потому что мне было лениво аккуратно парсить файл)
[+] добавлена возможность цепляться к уже запущенному процессу (эта возможность позволяет выбрать любой модуль в процессе для распаковки, но тут есть свои тонкости. если имя процесса в listbox полное с полным путём, то к процессу можно аттачиться, если же только имя файла, то для аттача прав не хватит. для данного метода нельзя указать руками OEP, за OEP считается инструкция, на которой программа стояла в момент аттача. для использования этого метода нужно загрузить интересующую программу в любой отладчик и руками дойти до OEP, а потом аттачиться к этому процессу QuickUnpack. следует помнить, что для использования опции smart import recovery не обязательно отпускать программу на выполнение, можно оставить её стоять в отладчике на бряке. но для использования smart import recovery with tracer необходимо поставить программу в бесконечную петлю (EB FE) и отпустить на выполнение, потому что QuickUnpack использует текущий поток для трассировки импорта. если программа была поставлена в петлю, не забудьте в дампе поправить эти 2 байта обратно. ну и трассировка импорта при аттаче довольно ненадёжная и медленная, поэтому использовать её нежелательно). эта опция позволяет использовать QuickUnpack как отдельный дампер и восстановитель импорта (попытка заменить PETools и ImpRec в 1 флаконе :))
[+] добавлена поддержка плагинов ImpRec (эта вещь позволяет использовать плагины от ImpRec в QuickUnpack для восстановления функций импорта. следует помнить, что при использовании аттача к процессу, для корректной работы трейсера программа должна быть отпущена на выполнение)
...
[26.08.2005]
v1.0 beta3 [+] Добавлена возможность распаковки длл
...
[10.01.2005]
v0.5 [+] QuickUnpack теперь использует новый движок и работает через внешний engine.sys. Debug API не используются
...
-----------------------
tPORt, AHTeam, TSRh, KpTeam, REVENGE, CRACKL@B, ICU мемберов, CoaxCable^CPH, Wild-Wolf и всех мемберов CPH на #cph, LaFarge[ICU] и SofT MANiAC за хорошую музыку, SergioPoverony, DillerInc, newborn за картинку, syd, lord_Phoenix, NCRangeR, Grim Fandango, Aster!x, Quantum, MozgC, Sten, PolishOX, NEOx, WELL, GPcH, Funbit, sl0n, Ms-Rem, Bad_guy, dj-siba, =TS=, DillerInc, UsAr, Human, deroko, Errins, Bronco, HandMill, Executioner, 4kusN!ck, BiT-H@ck, Hellsp@wn, HoBleen, Smon, LaZzy, Mikae...